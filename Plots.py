...
# model.load_state_dict(torch.load(file_loc + '/Models/FNO_single_blob_nullary-spot.pth', map_location=torch.device('cpu')))
...
for dim in range(num_vars):
    ...
    plt.savefig("single-blob_" + dims[dim] + "_complicated-nut.pdf", format="pdf", bbox_inches='tight', transparent='True')
...
for dim in range(num_vars):
    ...
    plt.savefig("single-blob_" + dims[dim] + "_complicated-nut_err.pdf", format="pdf", bbox_inches='tight', transparent='True')
...
for field in dims:
    ...
    if field == 'rho':
        model.load_state_dict(torch.load(file_loc + '/Models/FNO_single_blob_regular-cream.pth', map_location=torch.device('cpu')))
    if field == 'Phi':
        model.load_state_dict(torch.load(file_loc + '/Models/FNO_single_blob_pizzicato-meter.pth', map_location=torch.device('cpu')))
    if field == 'T':
        model.load_state_dict(torch.load(file_loc + '/Models/FNO_single_blob_plain-antenna.pth', map_location=torch.device('cpu')))
    model.to(device)
...
plt.savefig("single_blob_error_growth.pdf", bbox_inches='tight')
plt.savefig("single_blob_error_growth.svg", bbox_inches='tight')
# %%
