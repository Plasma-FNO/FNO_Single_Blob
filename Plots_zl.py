...
for field in dims:  
    ...
    if field == 'rho':
        model.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_contemporary-instance.pth', map_location=torch.device('cpu')))
    if field == 'Phi':
        model.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_auburn-sandcrab.pth', map_location=torch.device('cpu')))
    if field == 'T':
        model.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_vibrant-brandy.pth', map_location=torch.device('cpu')))
